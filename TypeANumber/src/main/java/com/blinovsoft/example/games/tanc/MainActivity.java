...
package com.blinovsoft.example.games.tanc;
...
public class MainActivity extends FragmentActivity
    ...
    // tag for debug loggingRC_UNUSED
    final boolean ENABLE_DEBUG = true;
    final String TAG = "TanC";
    final String TAG_MENU_FRAGMENT = "tag_menu_fragment";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Fragment fr = fm.findFragmentByTag(TAG_MENU_FRAGMENT);
        if(fr == null) {
            // add initial fragment (welcome fragment)
            fm.beginTransaction().add(R.id.fragment_container, mMainMenuFragment, TAG_MENU_FRAGMENT).commit();
        }
    ...
    void pushAccomplishments() {
        ...
        if (mOutbox.mArrogantAchievement) {
            Games.Achievements.unlock(mGoogleApiClient, getString(R.string.achievement_dont_get_cocky_kid));
        ...
        if (mOutbox.mLeetAchievement) {
            Games.Achievements.unlock(mGoogleApiClient, getString(R.string.achievement_omg_u_r_teh_uber_leet));
            mOutbox.mLeetAchievement = false;
        }
        if (mOutbox.mBoredSteps > 0) {
            Games.Achievements.increment(mGoogleApiClient, getString(R.string.achievement_really_really_bored),
                    mOutbox.mBoredSteps);
            Games.Achievements.increment(mGoogleApiClient, getString(R.string.achievement_bored),
                    mOutbox.mBoredSteps);
        }
        if (mOutbox.mEasyModeScore >= 0) {
            Games.Leaderboards.submitScore(mGoogleApiClient, getString(R.string.leaderboard_easy_high_scores),
                    mOutbox.mEasyModeScore);
            mOutbox.mEasyModeScore = -1;
        }
        if (mOutbox.mHardModeScore >= 0) {
            Games.Leaderboards.submitScore(mGoogleApiClient, getString(R.string.leaderboard_hard_high_scores),
...
/*        if (isSignedIn()) {
    ...
    private class ResetterTask extends AsyncTask<Void, Void, Void>
    {
        ...
        protected Void doInBackground(Void... params)
        {
            ...
            /*
            String acheivementid = getString(R.string.achievement_prime);
            HttpPost post = new HttpPost
                    (
                        "https://www.googleapis.com"+
                        "/games/v1management"+
...
//*/
        ...
        protected void onPostExecute(Void result)
        {
            ...
            //gui will look ok.
            PendingResult<Achievements.LoadAchievementsResult> achievementsResult = Games.Achievements.load(mGoogleApiClient, true);
    ...
    public void onSignInButtonClicked() {
        ...
        if(!BaseGameUtils.verifySampleSetup(this, R.string.app_id,
                R.string.achievement_prime, R.string.leaderboard_easy_high_scores)) {
